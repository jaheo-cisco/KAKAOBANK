...
def read_route_file() -> Optional[dict]:
    ...
    """
    print(f"Attempting to read last state of routing table from file: {LOCAL_FIL
E}")
    try:
        ...
        print(
            "No previous state was stored. Assuming script is new & file will be
...
def send_syslog(diff: dict) -> None:
    ...
    for route in diff["removed"]:
        ...
        syslog(
            LOG_ERR,
            f"ROUTE REMOVED - PREFIX: {prefix}, NEXT HOP: {route[prefix]['nextho
p']}, PROTOCOL: {route[prefix]['proto']}",
        )
    for route in diff["added"]:
        prefix = list(route.keys())[0]
        syslog(
            LOG_ERR,
            f"ROUTE ADDED - PREFIX: {prefix}, NEXT HOP: {route[prefix]['nexthop'
]}, PROTOCOL: {route[prefix]['proto']}",
        )
