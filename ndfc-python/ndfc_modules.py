...
def get_leaf_nodes(fabricName,raw=0):
    ...
    for index, leaf in enumerate(output):
        leaf_dict = {}
        if leaf['switchRole'].lower() == 'leaf' or leaf['switchRole'].lower() == 'border' or leaf['switchRole'].lower() == 'border gateway':
...
def get_switch_policy(serial_number):
    username = ndfc_credentials.username
    password = ndfc_credentials.password
    url = 'https://' + ndfc_credentials.node_ip
    posturl = url + f'/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/control/policies/switches/{serial_number}'
    ndfc_token = ndfc_auth.auth(ndfc_credentials.node_ip, username, password)
    headers = {'Authorization': ndfc_token, 'Content-Type': 'application/json'}
    payload = ''
...
def get_policy(fabricName,policyName):
    username = ndfc_credentials.username
    password = ndfc_credentials.password
    url = 'https://' + ndfc_credentials.node_ip
    posturl = url + f'/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/control/fabrics/{fabricName}'
    ...
    for node in leaf_nodes:
        policy_url = url + f'/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/control/policies/switches/{node["serial_no"]}'
        payload = ''
        ...
        for item in policy:
            try:
                if item['templateName']==policyName:
